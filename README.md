...
NPC instances follow one rule: **instances are deltas, not clones.** Spawning "Bandit" three times creates three panels with independent HP, temp HP, condition, **Action Points, and Recharge cooldowns**, while stats, abilities, portrait, and description are all read from the base record at render time. Editing the base updates every instance of it, and the NPC list keeps showing only bases — there is never a "which Bandit is the real one?" question.
...
| `kind: 'npc-instance'` | `{ id, baseNpcId, label, density, statuses, state }` where `state` is `{ currentHP, tempHP, condition, currentAP, cooldowns }`, `condition` is `'active' \| 'downed' \| 'dead'`, `currentAP` is the instance's own turn budget (spawned at 3), and `cooldowns` lists the ids of the base's abilities currently waiting on a Recharge Die |
