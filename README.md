...
`mortalWounds` is the instance's too, and it stores **the wounds themselves** (`{ roll, name }` — the D20 and the table entry) rather than a count, so the panel can print "Damaged Throat · d20 8" and the table's current rules text. The **allowance is deliberately not stored on the instance**: it is `base.npcStats.mortalWounds`, read at damage and render time like max HP, so raising a base's allowance applies to every instance already on the screen. A **player panel stores nothing extra at all**: its track is the character's own `mortalWounds` slots (two names, one per slot, `'Pending Roll'` while one is unresolved), which is why clearing a chip on the panel is visible on the player's sheet and survives a reload. `lib/mortalWounds.ts` owns the table lookup and the D20 roll for every surface — the player's `MortalWoundRoller`, an NPC instance's automatic roll, and a player panel's (`characterStore.takePanelDamage`) resolve through the same function, so a wound's name and description can never disagree.
...
│   │   ├── mortalWounds.ts  # The Mortal Wounds table roll (sheets + GM panels)
