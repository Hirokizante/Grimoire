...
- **Limited-use tracking** — an ability flagged as limited (see above) shows its remaining uses on its card and spends one per Activate when configured to. At zero uses the Activate button is disabled until a rest refills the budget, and unlimited abilities are completely unaffected.
- **GM Screen** — run several sheets at once on one surface, including each NPC instance's **own turn**: 3 AP, working Activate buttons, and Recharge cooldowns resolved by a Recharge Die roll. See [GM Screen](#gm-screen).
- **NPC Mortal Wounds on the GM Screen** — an NPC instance rolls on the Mortal Wounds table automatically when damage drives it to 0 HP, using the base's own **Mortal Wounds** stat as its allowance, and marks each wound (with its D20) on the panel. A base with `Mortal Wounds: 0` is untouched and still just goes Downed at 0 HP.
...
- **Compact / expanded** — compact is a glance-height card (portrait, name, HP bar, the **Action Point meter**, key stat tokens, condition badge); expanded renders the sheet content inline.
  ...
  - Deliberately omitted from the body, because the panel chrome already shows them: the **player HP bar** and the **Action Points bar** (both live under the panel header, with their own steppers and Damage dialog — printing them again inside the body would double every number the GM is watching), the **NPC Core Ability** section (NPCs have no core abilities — those fields only ever hold the generated Basic Attack / Fatebreaker defaults), and **Description / Character Background** on both (reference material, not at-the-table information).
...
- **Turns: Action Points & Recharge** — **every panel shows AP under its HP bar**, player and NPC alike, from one component (`PanelApBar`): the sheet's own segmented meter plus the panel's shared bar chrome, so HP and AP read as one control family — a label row (`HP 20/20`, `ACTION POINTS 3/3`) and a `[−] bar [+]` row with that row's action button (Damage… / Start new turn) at the end. Both bars render the same `PanelStepper` — lucide `−`/`+` glyphs in identical 1.75rem boxes — and disable each end exactly when its action would be a no-op (full HP, 0 AP), the rule the player sheet's bars use. An NPC instance is initialised with **3 AP per turn**; a player panel spends and restores the character's real AP, shared with their own sheet, and its AP token was dropped from the stat strip so the panel never prints the same number twice. Spending AP by hand is always available; activating an ability spends it automatically. **A panel whose entity is out of AP dims itself** — header, HP bar, tokens and the expanded sheet body — so a glance down the screen shows who has already acted. The AP block stays at full strength (it holds both ways back: the manual `+` and Start new turn), nothing is disabled or made un-clickable, and the panel returns to normal the instant AP does.
  ...
  - Nothing about a turn reaches the base NPC record: the standalone NPC sheet has **no Activate button, no AP meter, and no cooldowns** — it stays the static reference it has always been. Limited-use budgets (`uses`) also stay on the base record and render read-only in panels, exactly as they did before; the panel never spends another instance's uses. NPC abilities cost AP only (the NPC ability editor offers no END/FP costs), so the panel deducts AP and leaves untracked pools out of the tooltip rather than blocking on resources it cannot show.
...
- **Damage** — the same `DamageDialog` used on character sheets, targeting whichever panel opened it. Armor for an NPC instance comes from the base's `npcStats.armor` and max HP from `npcStats.hp`; temp HP is the instance's own. Reaching 0 HP follows the same Mortal Wound rule a player sheet does, with the base's own **Mortal Wounds** stat as the allowance (see below); healing a downed instance revives it, and the panel menu can flag it `dead` (which healing never clears).
- **NPC Mortal Wounds** — an NPC instance rolls on the Mortal Wounds table exactly as a player does, but the roll is automatic: damage that drives it to 0 HP rolls the D20 in the store, refills HP to max with the spill-over, and records the wound. There is no "Pending Roll" step and no second card to click — a GM running four bandits gets four wound results without leaving the damage dialog. The track is drawn in the panel chrome, right under the HP bar: `[skull Wounds n/max | chips | ⚠ Next 0 HP: Downed]`.
  - **How many wounds it can take is the base's `npcStats.mortalWounds`** — the editable stat on the NPC sheet (evasion/armor/movement/save DC/HP/Mortal Wounds), read at damage time like every other base stat, so editing the base updates every instance of it. The stat's tooltip says so on the sheet.
  ...
  - One hit can burn through several wounds: damage past 0 refills HP to max each time and costs a wound per refill, so a huge hit on a three-wound boss that still ends below 0 downs it.
  - Each chip carries the **D20 result** and the wound's name, with the full rules text in its tooltip — the same table data the player sheet's wound cards show. Wounds persist until cleared (the chip's own ✕, or the panel menu's **Clear mortal wounds**) or a Rest; **healing never clears them**, and the store never writes a wound to the base record.
  - The panel's own `−` stepper runs the same pipeline, so it can roll a wound too — that announces itself in a toast ("Bandit takes a Mortal Wound: Fracture (d20 14) — HP reset to 15."), because an HP bar that refills to max would otherwise look like nothing happened.
...
| `kind: 'npc-instance'` | `{ id, baseNpcId, label, density, statuses, state }` where `state` is `{ currentHP, tempHP, condition, currentAP, cooldowns, mortalWounds }`, `condition` is `'active' \| 'downed' \| 'dead'`, `currentAP` is the instance's own turn budget (spawned at 3), `cooldowns` lists the ids of the base's abilities currently waiting on a Recharge Die, and `mortalWounds` is the instance's own track of `{ roll, name }` wounds |
...
`currentAP` and `cooldowns` are the same kind of state: **the instance's, never the base's.** Three Bandits each spend their own AP and cool their own abilities down, and nothing a GM does at the table reaches the standalone NPC sheet. `cooldowns` stores bare ability ids rather than Recharge values, so the value is always read from the base's traits at roll time — editing a trait, or deleting the ability, can never leave a cooldown pinned to a stale number (an id that no longer resolves to a Recharge ability is dropped on the next roll). See `lib/abilityRecharge.ts`.

`mortalWounds` is the instance's too, and it stores **the wounds themselves** (`{ roll, name }` — the D20 and the table entry) rather than a count, so the panel can print "Damaged Throat · d20 8" and the table's current rules text. The **allowance is deliberately not stored on the instance**: it is `base.npcStats.mortalWounds`, read at damage and render time like max HP, so raising a base's allowance applies to every instance already on the screen. `lib/mortalWounds.ts` owns the table lookup and the D20 roll for both surfaces — the player's `MortalWoundRoller` and an NPC instance's automatic roll resolve through the same function, so a wound's name and description can never disagree.
...
| `addNpcInstancePanel` | Spawns at full HP **and a full turn (3 AP, nothing cooling)** with an auto-numbered label ("Bandit", "Bandit 2", …) |
| `duplicatePanel` | NPC instances only — spawns a *fresh* instance, never a copy of its HP, AP, or cooldowns |
...
│   │   │   ├── PanelMortalWounds.tsx   # An instance's wound track (auto-rolled)
...
│   │   ├── useNpcInstanceActivation.tsx # GM NPC live play: panel AP, Recharge cooldowns, turn roll
...
│   │   ├── mortalWounds.ts  # The Mortal Wounds table roll (sheets + NPC instances)
